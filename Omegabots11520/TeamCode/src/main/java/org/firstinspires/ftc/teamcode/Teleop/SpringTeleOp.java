...
// @TeleOp(name="SpringTeleOp", group="OmegaBots Opmode") // @Autonomous(...) is the other common choice
// @Disabled
public class SpringTeleOp extends LinearOpMode {
    ...
    double motorSpeedForBumpers = 0.4;
    double motorSpeedForDpadRotate = 0.8;
    double motorSpeedForLinearSlide = 0.4;// LINEAR SLIDE SPEED WHEN LOWERING
    double motorMaxSpeed = 1.0;
    double motorSpeedForRotation = 0.7;
    double bumperSpeed = 0.4; // set bumper speed for Straif
...
// DPAD Stuff
            ...
            } else if (gamepad1.right_bumper) { // Straif Left
                // motorMaxForStick1 = motorMaxForStick;
                robot.leftfront.setDirection(DcMotor.Direction.REVERSE);
                robot.rightfront.setDirection(DcMotor.Direction.FORWARD);
                robot.leftrear.setDirection(DcMotor.Direction.FORWARD);
                robot.rightrear.setDirection(DcMotor.Direction.REVERSE);
                runMotors(motorSpeedForBumpers);
                //resetEncoders();
            } else if (gamepad1.left_bumper) { // Straif Right
                // motorMaxForStick1 = motorMaxForStick;
                robot.leftfront.setDirection(DcMotor.Direction.FORWARD);
                robot.rightfront.setDirection(DcMotor.Direction.REVERSE);
                robot.leftrear.setDirection(DcMotor.Direction.REVERSE);
...
/*            if (gamepad2.y) { // Gamepad vertical up button
                ...
                robot.linearSlide.setDirection();
...
*///-------------------------------CLAW OPEN CLOSE -------------------------------------------------
            ...
            if (gamepad2.left_bumper) {
               // robot.clawL.setPosition(0.7);
               // robot.clawR.setPosition(0.4);
                robot.clawL.setPosition(0.6);
                robot.clawR.setPosition(0.5); //connor's idea on fixing claw :)
...
//---------------------------LINEAR SLIDE UP DOWN STUFF---------------------------------------------
            ...
            // Set the height we want using the Integers
            JunctionLow = 900;
            JunctionMedium = 1600;
            JunctionHigh = 2400;
            Floor = 80;

            // Tell the motor to chill out an be cool were going to use some encoders
            ...
            // Lets make it all move now
            if (gamepad2.a) {
                do { //Low Junction
                    robot.linearSlide.setPower(0.5);
                    robot.linearSlide.setTargetPosition(JunctionLow);
                    robot.linearSlide.setMode(DcMotor.RunMode.RUN_TO_POSITION);
                } while (gamepad2.a);
            }
            if (gamepad2.b) {
                do { // Medium Junction
                    robot.linearSlide.setTargetPosition(JunctionMedium);
                    robot.linearSlide.setPower(0.5);
                    robot.linearSlide.setMode(DcMotor.RunMode.RUN_TO_POSITION);
                } while (gamepad2.b);
            }
            if (gamepad2.y) {
                do { // High Junction
                    robot.linearSlide.setTargetPosition(JunctionHigh);
                    robot.linearSlide.setPower(0.5);
                    robot.linearSlide.setMode(DcMotor.RunMode.RUN_TO_POSITION);
                } while (gamepad2.y);
            }
            if (gamepad2.x) {
                do { // Back to the Floor please
//                robot.linearSlide.setTargetPosition(Floor);
                    robot.linearSlide.setDirection(DcMotor.Direction.FORWARD);
                    robot.linearSlide.setPower(0.4);
//                    robot.linearSlide.setMode(DcMotor.RunMode.RUN_TO_POSITION);
//                robot.linearSlide.setPower(0.0);
                    runLinearMotor(motorSpeedForLinearSlide);
//                    robot.linearSlide.setZeroPowerBehavior(DcMotor.ZeroPowerBehavior.BRAKE);
                } while (gamepad2.x);
            }
            else {
...
//        robot.linearSlide.setPower(power);
    ...
    private void runLinearMotor( double power){
        robot.linearSlide.setPower(power);
    }
